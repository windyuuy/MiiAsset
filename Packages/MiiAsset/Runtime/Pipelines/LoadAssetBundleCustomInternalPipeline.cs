	...
	namespace MiiAsset.Runtime.Pipelines
	{
		public class LoadAssetBundleCustomInternalPipeline : ILoadAssetBundlePipeline, IDownloadPipeline
		{
			...
			private async Task<PipelineResult> _run()
			{
				...
				try
				{
					if (!string.IsNullOrEmpty(CacheUri) && await IOManager.LocalIOProto.ExistsAsync(CacheUri))
					{
						CachedAssetBundle = await IOManager.LocalIOProto.ReadAllBytesAsync(CacheUri, bytes =>
						{
							...
							else
							{
								...
								if (assetBundle == null)
								{
									// var exist = await IOManager.LocalIOProto.ExistsAsync(CacheUri);
									MyLogger.LogError(
										$"Failed to Load AssetBundle firstTime1: {CacheUri}, {bytes.Length}");
								}
			...
			private void DeleteCachedFile()
			{
				if (!string.IsNullOrEmpty(CacheUri))
				{
					...
					// if (exist)
					{
						IOManager.LocalIOProto.DeleteAsync(CacheUri);
					}
				}
			}
			...
			public AssetBundle AssetBundle => CachedAssetBundle;
